            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white/5">
                    ...
                    >
                        <div className="chat-image">
                            <div
                                ...
                                style={{ backgroundColor: message.sender_ID === authUser.id ? authUser.userIdColor : selectedUser.userIdColor }}
                            >
                                {message.sender_ID === authUser.id ? authUser.username[0].toUpperCase() : selectedUser.username[0].toUpperCase()}
                            </div>
                        </div>

                        <div className="chat-header mb-1">
                            {message.sender_ID === authUser.id ? authUser.username : selectedUser.username}
                            <time className="text-xs opacity-50 ml-1">{formatMessageTime(message.created_at)}</time>
                        </div>
                ...
                {isTyping && (
                    <div className={`chat chat-start`}>
                        <div className="chat-image">
                            <div
                                className={`size-12 rounded-full flex items-center justify-center text-primary-content bg-black bg-opacity-20 font-medium shadow-inner shadow-green-700 border-green-500 border-2`}
                                style={{ backgroundColor: selectedUser.userIdColor }}
                            >
                                {selectedUser.username[0].toUpperCase()}
                            </div>
                        </div>
                        <div className="chat-bubble flex bg-black/50 items-center justify-center w-16 h-8">
